...
export default function MessageBubble({ message }) {
    ...
    return (
        <div className={cn("flex gap-3", isUser ? "justify-end" : "justify-start")}>
            {!isUser && (
                <div className="h-8 w-8 rounded-lg bg-gradient-to-r from-yellow-400 to-yellow-600 flex items-center justify-center mt-0.5 flex-shrink-0">
            ...
            <div className={cn("max-w-[85%]", isUser && "flex flex-col items-end")}>
                {message.content && (
                    <div className={cn(
                        ...
                        isUser ? "bg-gray-800 text-white" : "bg-gray-800/50 border border-gray-700 text-gray-100"
                    )}>
                        {isUser ? (
                                ...
                                components={{
                                    code: ({ inline, className, children, ...props }) => {
                                        ...
                                        return !inline && match ? (
                                            ...
                                            <code className="px-1 py-0.5 rounded bg-gray-700 text-yellow-400 text-xs">
                                                {children}
                                            </code>
                                        );
                                    },
                                    a: ({ children, ...props }) => (
                                        <a {...props} target="_blank" rel="noopener noreferrer" className="text-yellow-400 hover:text-yellow-300">{children}</a>
                                    ),
                                    ...
                                    blockquote: ({ children }) => (
                                        <blockquote className="border-l-2 border-yellow-500 pl-3 my-2 text-gray-300">
                                            {children}
                                        </blockquote>
                                    ),
                                }}
            ...
            {isUser && (
                <div className="h-8 w-8 rounded-lg bg-gray-700 flex items-center justify-center mt-0.5 flex-shrink-0">
